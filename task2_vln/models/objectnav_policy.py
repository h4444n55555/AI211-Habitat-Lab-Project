...
class ObjectNavPolicy(nn.Module):
    """
    Policy for Object-Goal Navigation (ObjectNav).
    - Vision encoder: CLIP vision transformer
    - Goal encoder: Embedding layer for discrete object categories
    ...
    def __init__(
        ...
        fusion_dim: int = 512,
        num_object_categories: int = 21,  # e.g., MP3D typical 21 categories
        num_actions: int = 4,
        dropout: float = 0.1,
    ):
        super().__init__()
        ...
        self.goal_emb = nn.Embedding(num_object_categories, fusion_dim)
    ...
    def forward(self, pixel_values: torch.Tensor, object_id: torch.Tensor):
        """
        pixel_values: [B, C, H, W]
        object_id: [B] (integer class of the target object)
        """
        vision_out = self.vision_encoder(pixel_values=pixel_values)
        vision_feat = self.vision_proj(vision_out.pooler_output)
        
        goal_feat = self.goal_emb(object_id)
